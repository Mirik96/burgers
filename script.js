...
let btnPlusOrMinus = document.querySelectorAll('.main__product-btn'),
    ...
    mainInfo = document.querySelectorAll('.main__product-info'),
    ...
    viewClose.addEventListener('keyup', function(el) {
        if(el.keyCode == 13) {
            view.classList.remove('active');
        }
    })
...
function out (num, elem) {
    let timeOut = document.querySelector(elem),
    n = 0
...
addCart.addEventListener('click', function() {
    for(const key in product) {
        ...
        if(burgersName.amount > 0) {
            ...
            for(const newKey in burgersName) {
...
addCart.addEventListener('keyup', function(el){
    if(el.keyCode === 13){
        location.reload();
    }
})
